...
class List
{
	...
	public:
		...
		{ // Iterate thorugh list once, replace and increment on matches
				int counter = 0;
				for (iterator itr = begin(); itr != end(); itr++) {
					if (itr.current->data == replacee) {
						itr.current->data = replacer;
						counter++;
					}
				}
				return counter;
		...
		int removeAll (const Object & deletee) // t = O(n), m = O(n)
		{ // Iterate thorugh list once, replace and increment on matches
				int counter = 0;
				for (iterator itr = begin(); itr != end(); ) {
					if (itr.current->data == deletee) {
						itr = erase(itr);
						counter++;
					}
					else {
